...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // As downloaded from https://www.kaggle.com/bileschi/account?isEditing=False
  const kaggleKey = {
    'username': 'bileschi',
    'key': '9584962dea5bc58556e13bc2ff53af4a'
  };
  const preEncodeKey = kaggleKey.username + ':' + kaggleKey.key;
  const auth = 'Basic ' + btoa(preEncodeKey);

  const URL =
      'https://www.kaggle.com/api/v1/datasets/download-raw/unsdsn/world-happiness/2015.csv';
  const requestInfo = new Request(URL);
  requestInfo.headers.append('Authorization', auth);

  console.log(preEncodeKey);
  console.log(auth);
  console.log("Try fetch style 1");
  fetch(requestInfo, {mode: 'no-cors'}).then(result => {
    result.text().then(v => {console.log('try 1:' + console.log(v))});
  });
  console.log("Try fetch style 2");
  fetch(URL, {
    method: 'GET',
    ...
    headers: {
      ...
      result.text().then(v => {console.log('try 2:' + console.log(v))});
    });
  // fetch(requestInfo, {mode: 'cors'}).then(result => {
  //  result.text().then(v => {console.log('try 2' + console.log(v))});
  //});
  // fetch(requestInfo).then(result => {
  //   result.text().then(v => {console.log('try 3' + console.log(v))});
  // });
}, false);
