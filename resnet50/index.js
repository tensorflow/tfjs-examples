...
let resnet;
const resnetDemo = async () => {
  status('Loading model...');

  resnet = await tf.loadLayersModel(RESNET_MODEL_PATH);
...
/**
 * Given an image element, makes a prediction through resnet returning the
...
async function predict(imgElement) {
  ...
  const logits = tf.tidy(() => {
    ...
    // Below preprocessing is copied from: http://deeplizard.com/learn/video/hRKEQhiqIU4
    const meanResNetRGB = {
        red: 103.939,
        green: 116.779,
        blue: 123.68
    ...
    const centeredRGB = {
        red: tf.gather(tensor, indices[0], 2)
            .sub(tf.scalar(meanResNetRGB.blue))
            .reshape([IMAGE_SIZE*IMAGE_SIZE]),
        green: tf.gather(tensor, indices[1], 2)
            .sub(tf.scalar(meanResNetRGB.green))
            .reshape([IMAGE_SIZE*IMAGE_SIZE]),
        blue: tf.gather(tensor, indices[2], 2)
            .sub(tf.scalar(meanResNetRGB.red))
            .reshape([IMAGE_SIZE*IMAGE_SIZE])
    };
    const batched = tf.stack([
    ...
    // Make a prediction through resnet.
